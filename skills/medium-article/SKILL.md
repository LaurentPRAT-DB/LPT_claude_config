---
name: medium-article
description: Create professional Medium articles from GitHub repositories with Mermaid diagrams, Gist code embeds, and GitHub Pages publishing. Includes complete workflow from repo analysis to Medium import.
---
...
### ⚠️ CRITICAL: Working Directory Hygiene

**Before generating ANY diagrams, ALWAYS clean your working directory:**
```bash
# Clean shared working directory OR use project-specific directory
rm -rf ~/medium_article_images/*
# OR better: use project-specific directory
mkdir -p ~/medium_article_images/$(basename $PWD)
```

**Why:** Previous projects leave images in `~/medium_article_images/`. The `cp *.png` command will copy ALL images, including ones from other projects. This causes repo pollution with unrelated files.

**Rule:** Generate ONLY diagrams specific to the current article. Never reuse images from other projects.
...
### Lesson 9: Working Directory Hygiene - Only Generate Project-Specific Images
When creating diagrams, images from previous projects may exist in working directories like `~/medium_article_images/`. These can accidentally get copied to the new project's repo.

**Problem:** In this session, images from the ACME Workspace Usage Dashboard (`burndown_chart.png`, `sweet_spot.png`, etc.) were accidentally committed to the cluster-manager repo.

**What went wrong:**
- The `cp ~/medium_article_images/*.png REPO/docs/images/` command copied ALL images, not just project-specific ones
- Images from previous articles were still in the working directory

**Prevention:**
1. **ALWAYS clean the working directory** before generating images for a new article:
   ```bash
   rm -rf ~/medium_article_images/*
   ```
2. **Only generate Mermaid diagrams specific to the current project** - never reuse images from other projects
3. **Verify image list before committing** - check `git status` and confirm each file is project-relevant
4. **Use project-specific working directories** instead of a shared one:
   ```bash
   mkdir -p ~/medium_article_images/cluster-manager
   mmdc -i diagram.mmd -o ~/medium_article_images/cluster-manager/output.png
   ```

**Rule:** Generate ONLY project-specific Mermaid diagrams. Never copy images blindly from shared working directories.
