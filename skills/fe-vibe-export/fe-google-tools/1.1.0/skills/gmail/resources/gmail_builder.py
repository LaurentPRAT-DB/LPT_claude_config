...
def get_access_token() -> str:
    ...
    if result.returncode != 0:
        print("Failed to get access token. Run: python3 gmail_auth.py login", file=sys.stderr)
        sys.exit(1)
    return result.stdout.strip()


def api_request(method: str, endpoint: str, data: Optional[Dict] = None) -> Dict:
    """Make an authenticated API request to Gmail."""
...
def create_draft(to: str, subject: str, body: Optional[str] = None,
                 html: Optional[str] = None, cc: Optional[str] = None,
                 bcc: Optional[str] = None, attachments: Optional[List[str]] = None) -> Dict:
...
def reply_message(message_id: str, body: Optional[str] = None,
    ...
    if reply_all:
        ...
        for addr in (orig_to + ',' + orig_cc).split(','):
            addr = addr.strip()
            if addr and 'me' not in addr.lower():
                all_recipients.add(addr)
        all_recipients.discard(orig_from)
        if all_recipients:
