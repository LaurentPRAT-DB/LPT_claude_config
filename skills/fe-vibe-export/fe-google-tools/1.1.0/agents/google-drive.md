...
```bash
python3 ../skills/google-docs/resources/gdocs_auth.py status
```

If authentication fails or scopes are missing, run:

```bash
python3 ../skills/google-docs/resources/gdocs_auth.py login
```
...
### 1. Authentication Manager (`gdocs_auth.py`)
...
# Get access token for custom curl calls
python3 ../skills/google-docs/resources/gdocs_auth.py token

# Validate scopes
python3 ../skills/google-docs/resources/gdocs_auth.py validate
```
...
```bash
TOKEN=$(python3 ../skills/google-docs/resources/gdocs_auth.py token)
...
1. **Check auth first** - Run `gdocs_auth.py status` before any operation
2. **Token expires after 1 hour** - Re-run `gdocs_auth.py login` if needed
3. **Quota project required** - All calls need `gcp-sandbox-field-eng` header
...
| Error | Solution |
|-------|----------|
| "Token expired" | Run `gdocs_auth.py login` |
| "Insufficient scopes" | Run `gdocs_auth.py login --force` |
