...
def update_environments_from_api(deployments: List[Dict[str, Any]]):
    ...
    Args:
        deployments: List of deployment dicts from /api/deployments
    """
    workspaces = []

    for d in deployments:
        # Parse deployment outputs to get workspace URL
        workspace_url = None
        workspace_id = None
        outputs = d.get("deployment_outputs", "")

        if outputs:
            # Try parsing as JSON first (new format)
            try:
                outputs_json = json.loads(outputs)
                workspace_url = outputs_json.get("workspace_url")
                workspace_id = outputs_json.get("workspace_id")
            except (json.JSONDecodeError, TypeError):
                # Fall back to Terraform-style parsing (old format)
                # Format is multi-line like:
                # workspace_url: {
                #   value: https://...
                # }
                import re
                url_match = re.search(r'workspace_url:\s*\{[^}]*value:\s*(\S+)', outputs)
                if url_match:
                    workspace_url = url_match.group(1)
                id_match = re.search(r'workspace_id:\s*\{[^}]*value:\s*(\d+)', outputs)
                if id_match:
                    try:
                        workspace_id = int(id_match.group(1))
                    except ValueError:
                        pass

        # Determine workspace type from template name
        template = d.get("template_names", "").lower()
        if "serverless" in template:
            workspace_type = "serverless"
        elif "classic" in template:
        ...
        workspace = {
            ...
            "region": d.get("region"),
            "template": d.get("template_names"),
            "workspace_type": workspace_type,
            "created_at": d.get("created_at"),
            "expected_deletion_date": d.get("expected_deletion_date"),
