...
| Element | Requirement |
|---------|-------------|
| Headings | Use Google Docs heading styles (TITLE, HEADING_1, HEADING_2) |
| Bullet lists | Use `createParagraphBullets` API, NOT text bullet characters |
| Links | Embed hyperlinks in text using `updateTextStyle` with link |
| Slack channels | Link to channel archive URL |
| Google Docs | Link to document URL |
| Public docs | Link to docs.databricks.com URL |
| People | Include ALL people mentioned in answer, with titles if known |
| Citations | Inline citations for key facts (e.g., "claim [Public Docs: page]") |
