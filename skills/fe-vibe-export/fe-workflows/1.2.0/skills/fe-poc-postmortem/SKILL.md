...
### Slack Sources
If the user provides Slack channels/threads as sources, the agent will first validate MCP access using the `validate-mcp-access` skill. The user must have active credentials for `slack-mcp` before proceeding.
