...
**IMPORTANT: Before running any discovery, validate Slack MCP access using the validate-mcp-access skill.**

### Step 1: Run the validate-mcp-access skill
...
This skill will:
1. Get the user's Databricks identity from logfood workspace
2. Check if Slack MCP credentials are active
3. Report the status clearly

### Step 2: Handle Authentication Issues

If the skill reports that Slack credentials are **NOT CONFIGURED** or expired:

1. **Inform the user** that their Slack MCP access token has expired or is not configured
2. **Provide the login URL**:
   ```
   https://adb-2548836972759138.18.azuredatabricks.net/explore/connections/slack-mcp?o=2548836972759138
   ```
3. **Walk them through the process**:
   - Click the URL above
   - Complete the OAuth flow in the browser (authorize Databricks to access Slack)
   - Return to the conversation once complete
4. **Re-run validation** after they confirm authentication is complete
5. **Only proceed** once the skill shows Slack credentials as **ACTIVE**

### Common Authentication Errors

| Error | Meaning | Solution |
|-------|---------|----------|
| `invalid_code` | Token refresh failed | Re-authenticate via URL |
| `Credential not found` | Never authenticated | Authenticate via URL |
| Access token expired | Token needs refresh | Usually auto-refreshes; if not, re-authenticate |

**Do NOT proceed with discovery until Slack MCP access shows ACTIVE status.**
